...
import { CalendarDays, ChevronRight, FileCheck2, GraduationCap, LogOut, Search, ShieldCheck, Sparkles, Users } from "lucide-react";
...
export default function HomePage() {
  ...
  return <main className="app-shell">
    <header className="topbar"><div className="brand-row"><div className="brand-mark small">BI</div><div><strong>Bina Insan LifeMap</strong><span>Student Life & Career Dashboard</span></div></div><div className="top-actions"><div className="search-box"><Search size={18}/><input placeholder="Cari siswa, NIS, karier..." value={query} onChange={(e)=>setQuery(e.target.value)}/></div><button className="icon-btn" onClick={()=>supabase.auth.signOut()} title="Keluar"><LogOut size={18}/></button></div></header>
    <div className="workspace">
      <section className="main-column">
        <div className="hero-row"><div><p className="eyebrow">TAHUN AJARAN 2026/2027</p><h1>Student Life & Career Map</h1><p className="muted">Pantau 116 siswa dari kelas X hingga XII, proposal hidup, dan arah perkembangan masing-masing.</p></div><span className="status-pill"><ShieldCheck size={16}/> Supabase aktif</span></div>
      ...
      <aside className="side-column"><div className="side-card accent-card"><p className="eyebrow">RINGKASAN</p><h2>{students.length} siswa aktif</h2><p>Empat rombel dari kelas X, XI, dan XII sudah terhubung. Proposal Word/PDF sekarang dapat dikelola langsung dari profil siswa.</p></div><div className="side-card"><div className="side-title"><CalendarDays size={18}/><strong>Progress Tahap 2</strong></div><div className="class-breakdown"><div><span>Kelas X</span><strong>{counts.x}</strong></div><div><span>Kelas XI</span><strong>{counts.xi}</strong></div><div><span>Kelas XII</span><strong>{counts.xii}</strong></div><div><span>Proposal</span><strong>{proposalCount}</strong></div></div></div></aside>
    </div>
